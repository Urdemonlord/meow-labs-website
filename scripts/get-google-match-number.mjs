...
await target.locator('input[name="Passwd"]').fill(PASSWORD, { force: true })
await target.getByRole('button', { name: /^Next$/i }).click({ force: true })
...
await target.screenshot({ path: '/tmp/google-match-number-browser.png', fullPage: true })
console.log('SCREENSHOT:/tmp/google-match-number-browser.png')
await browser.close()
